...
LexBuffer::EAdvanceResult LexBuffer::GetCurrentChar(char& outChar) const{
  ...
  //EofPos may be the same as BufferSize, so do it first or else we could incorrectly return eoc
  if (Pos+1 == EofPos) {
    return eof;
  }
  if (Pos+1 == BufferSize) {
...
LexBuffer::EAdvanceResult LexBuffer::AdvanceMany(uint32_t num, std::vector<char> &outChars)
{
#ifndef NDEBUG
...
LexBuffer::EAdvanceResult LexBuffer::Peek(uint32_t offset, char& peekedChar) const {
...
uint32_t LexBuffer::GetRemainingElementSpace() const {
  uint32_t t = BufferSize - Pos;
  return t;
}
...
void LexBuffer::SetEOF(const streamsize streamSize) {
  EofPos = streamSize;
}

Lexer::Lexer() : LexerBuffer(0) {
  PopulateSymbolTree();
}

Lexer::Lexer(const std::vector<std::string>& files, size_t lexBufferSize) : LexerBuffer(lexBufferSize) {
  PopulateSymbolTree();
  ...
  for (auto& file : files) {
    ...
    while (true) {
      if (ofile.read(LexerBuffer.GetWritingPosition(), LexerBuffer.GetWritingAmount()) || ofile.gcount() > 0) {
        //If we have exactly enough chars to fill the buffer, this will never trigger.
        if (ofile.eof() || ofile.tellg() == size) {
          LexerBuffer.SetEOF(ofile.gcount()+LexerBuffer.PeekOffset);
        }
...
void Lexer::CreateToken(tok::TokenKind tokKind, bool bSaveLiteral) {
  if (bSaveLiteral) {
    TokenBuffer.emplace_back(tokKind, string(ChunkBuffer.begin(), ChunkBuffer.end()));
  } else {
...
void Lexer::MarchWord() {
  ...
  while (isalnum(curChar)) {
    ...
    //We do this because we always want to advance but we dont want to override the
    //previous result.
    LexBuffer::EAdvanceResult tempResult = LexerBuffer.Advance(curChar);

    if ( advResult == LexBuffer::EAdvanceResult::eoc) {
      //Don't make token or reset march state, we were cut off, wait for buffer refill
      return;
    }
  ...
  if (IdentifierTable.IsIdentifier(ChunkBufferToStringView(),/*OUT*/ keywordToken)) {
...
void Lexer::MarchSymbols() {
   ...
   *have to take that into account. We do so with peeking, which no other march function
   *does.
   *
  ...
  while (issymbol(curChar)) {
    ...
    if (curNode) {
      if (curNode->children.empty()) {
        ...
        if (curNode->bIsIdentifier) {
          ...
          throw std::logic_error("Symbol was the last element in trie chain yet it isnt marked identifier!!!");
        }
      } else {
        if (curNode->bIsIdentifier) {
          biggestPeekedIdentifierPos = numOfPeeks;
      ...
      if (biggestPeekedIdentifierPos == INDEX_NONE) {
        //We do this so we can advance, otherwise we would advance INDEXNONE(aka -1) + 1 = 0
        ...
        BrainFreeze::SyntaxError newError("Invalid symbol: " + std::string(ChunkBuffer.begin(), ChunkBuffer.end()),
          BrainFreeze::ErrorContext(TargetFileName, TargetFilePath, CurrentLine + 1, CurrentColumn - ChunkBuffer.size()));
    ...
    LexBuffer::EAdvanceResult peekResult = LexerBuffer.Peek(++numOfPeeks, curChar);

    if (peekResult == LexBuffer::eoc) {
      //We tried to peek but we went out of bounds, wait for the buffer to refill
  ...
  if (IdentifierTable.IsIdentifier(ChunkBufferToStringView(), /*OUT*/ tokenKind)) {
    ...
    if (tokenKind == tok::TokenKind::slashslash) {
      ...
      // We detected a string, discard these symbols and start marching the string
      ChunkBuffer.clear();
...
int32 Lexer::Lex(const std::string &targetFileName, const std::string &targetFilePath) {
