...
bool Lexer::March(char& c, bool saveToChunkBuffer) {
  ...
  if (saveToChunkBuffer){ChunkBuffer.push_back(Start[HeadIndx]);}
...
void Lexer::MarchSymbols() {
  /*Marching symbols is a bit more complicated that others because some symbols can be a combo of punctuators.
   *For e.g = is a token but so is == . So we have to take that into account
   *
   *Also, the other march functions will try to completely resolve their string, but in this func
   *it is possible to leave even if there are symbols left. This is too simplify the function. Our main loop will
   *handle this and call this function again if that is the case
   *
   *Regardless our rule still applies that we will leave the function with the chunk buffer empty
   *and the head at the next symbol to march.
   **/
  char curChar = GetCurrChar();
  ...
  auto makeSymbolToken = [this]() {
    ...
    if (IdentifierTable.IsIdentifier(ChunkBufferToStringView(), /*OUT*/ tokenKind)) {
      ...
      //TODO: finish
      BrainFreeze::Error newError(std::string("Symbols dont make up an identifier"),
        BrainFreeze::ErrorContext("", "", CurrentLine, CurrentColumn - ChunkBuffer.size()));

      std::cout<<std::format("Symbols dont make token!!!{}",ChunkBufferToStringView())<<std::endl;
    }
  };
    ...
    if (curNode) {
      if (curNode->children.empty()) {
        ...
        if (curNode->bIsIdentifier) {
          ...
          //TODO compiler error
          std::cout<<"Symbol was the last element in trie chain yet it isnt marked identifier!!!"<<std::endl;
        }
      }else {
        if (curNode->bIsIdentifier) {
      ...
      if (iterSinceLastSeenKeyword != numOfMarches) {
        ...
        ChunkBuffer.erase(ChunkBuffer.end() - iterSinceLastSeenKeyword, ChunkBuffer.end());
        bCompletedLex = false;
        makeSymbolToken();
      }else {
...
void Lexer::MarchCommentLine() {
  ...
  GenMarch(func);
  NewLine();
  //WARNING: If we ever change this to where we don't call create token, be sure to clear the chunk buffer
